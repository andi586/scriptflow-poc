...
// ── Supabase client (anon key — browser-side, public bucket) ─────────────────
...
export default function AppFlowPage() {
  ...
  // ── helper: upload blob DIRECTLY to Supabase Storage (bypasses Vercel 4.5MB limit) ──
  const uploadBlob = useCallback(async (blob: Blob, filename: string): Promise<string | null> => {
    try {
      ...
      console.log("[app-flow] uploading directly to Supabase:", filePath, "size:", blob.size, "type:", contentType);
      ...
      if (error) {
        console.warn("[app-flow] Supabase direct upload error:", error.message);
        return null;
      }

      const { data: publicData } = supabase.storage.from("recordings").getPublicUrl(data.path);
      const publicUrl = publicData.publicUrl;
      console.log("[app-flow] uploaded", filename, "→", publicUrl);
      return publicUrl;
    } catch (e) {
