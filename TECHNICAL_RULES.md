...
**发现日期：2026年4月15日，Day 30**

### 实测数据
- Scene镜头：Kling生成 = 2-3分钟
- Face镜头：Kling + OmniHuman + Shotstack = 6-10分钟
- 用户等待时间 = 最慢的Face镜头完成时间

### 必须遵守的优化规则

**规则1：Face镜头最优先提交**
提交任务时，所有Face镜头必须在Scene镜头之前提交。
不允许Scene和Face交替提交。

**规则2：场景预览推送**
当所有Scene镜头完成时，立刻推送通知用户：
"你的电影场景已就绪，正在生成你的数字人演员..."
不要让用户在黑暗中等待。

**规则3：OmniHuman缓存（待实现）**
数字分身建立后，预生成10个标准情绪表情视频缓存。
生成电影时优先复用缓存，减少OmniHuman调用次数。
目标：把Face镜头生成时间从6-10分钟压缩到1-2分钟。

**规则4：用户等待体验**
等待界面必须实时显示每个镜头的进度，不能只显示转圈。
用户需要知道"第3个镜头正在生成，预计还需要4分钟"。
